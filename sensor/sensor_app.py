...
print("\nHouse Temperature sensor recorders for area {} = {}".format(test_area, len(recs)))
print("\tMaximum: {0}, Minimum: {1} temperature".format(max(recs), min(recs)))

recs = temperature_data.get_data_by_date(rec_date=test_date)
print("\nHouse Temperature sensor recorders for date: {} = {}".format(test_date.strftime("%m/%d%y"), len(recs)))
print("\tMaximum: {0}, Minimum: {1} temperature".format(max(recs), min(recs)))
# Module 4 code here:
